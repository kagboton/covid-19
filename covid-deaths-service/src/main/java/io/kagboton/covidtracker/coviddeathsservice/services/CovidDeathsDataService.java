...
import io.kagboton.covidtracker.coviddeathsservice.models.LocationStats;
...
@Service
public class CovidDeathsDataService implements ICovidDeathsDataService {
    ...
    @Override
    public List<CountryDeathsStats> fetchDataAndConstructFinalCountryDeathsStats() throws IOException, InterruptedException {

        List<CountryDeathsStats> countryDeathsStatsList = new ArrayList<>();
        ...
        for (CSVRecord record : records) { //For each line of the confirmed case table
            ...
            int totalDeathsUntilPresentDay = Integer.parseInt(record.get(record.size() - 1));
            int totalDeathsUntilDayBeforePresentDay = Integer.parseInt(record.get(record.size() - 2));
            int newDeaths = totalDeathsUntilPresentDay - totalDeathsUntilDayBeforePresentDay;

            locationStats.setCountry(country);
            locationStats.setSlug(slug);
            locationStats.setTotalDeaths(totalDeathsUntilPresentDay);
            locationStats.setNewDeaths(newDeaths);

            locationStatsList.add(locationStats);

        }

        /*Construction of CountryDeathsStats list*/
        for (Country country : allWordCountries) {
            //Initialisation of the CountryDeathsStats objects
            CountryDeathsStats countryDeathsStats = new CountryDeathsStats();
            countryDeathsStats.setCountry(country.getCountry());
            countryDeathsStats.setSlug(country.getSlug());
            countryDeathsStats.setTotalDeaths(0);
            countryDeathsStats.setNewDeaths(0);

            for (LocationStats locationStats : locationStatsList){
                //Data update
                if(locationStats.getSlug().equals(countryDeathsStats.getSlug())){
                    int otherLinesRecovered = countryDeathsStats.getTotalDeaths();
                    int newLineRecovered = locationStats.getTotalDeaths();
                    countryDeathsStats.setTotalDeaths(otherLinesRecovered + newLineRecovered);

                    int otherLinesNewRecovered = countryDeathsStats.getNewDeaths();
                    int newLineNewRecovered = locationStats.getNewDeaths();
                    countryDeathsStats.setNewDeaths(otherLinesNewRecovered + newLineNewRecovered);
                }
            }

            countryDeathsStatsList.add(countryDeathsStats);

        }

        return countryDeathsStatsList;
